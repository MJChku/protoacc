...
case class VCRParams() {
  val nCtrl = 1            // Number of control registers (e.g., 'launch' signal)
  val nECnt = 1            // Number of event counters (if any)
  val nVals = 2            // Number of 32-bit value registers (min_fieldno, max_fieldno)
  val nPtrs = 10           // Increase to 10 to hold 5 64-bit pointers
  val nUCnt = 1            // Number of user counters (if any)
  val regBits = 32         // Register bit-width (keep as 32 bits)
}
